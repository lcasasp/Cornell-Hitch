...
#Not ideal for security, but this env makes it so we can run authentication on http instead of https (okay for hackathon purposes).
...
#note: docstring code represents flask_login authentication that is needed to logout users and 
#keep track of their authentication status, however, for the purpose of this hackathon we will treat any 
#google user to be permanently authorized for the app.
...
def callback():
    ...
    #optional, if statement can be removed if non-verified google users are allowed.
